...
class TaskAgentNode(BaseNode):
    ...
    async def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Mapa temporal para guardar las llamadas pendientes: {call_id: {name, args}}
            pending_calls = {}

            for msg in scratchpad:
                # 1. Capturar intenciones (AIMessage con tool_calls)
                    ...
                    for call in msg.tool_calls:
                        ...
                        if call_id:
                            pending_calls[call_id] = {
                                "name": call.get("name"),
                                "args": call.get("args") # Esto suele ser un dict ya parseado
                            }

                # 2. Capturar resultados (ToolMessage) y cruzar con la intención
                elif isinstance(msg, ToolMessage):
                    call_id = msg.tool_call_id
                    call_info = pending_calls.get(call_id, {})

                    tool_name = call_info.get("name", msg.name or "UnknownTool")
                    # Intentar serializar args a JSON string si es dict, para consistencia con TaskerBot
