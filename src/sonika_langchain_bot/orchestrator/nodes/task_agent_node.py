...
class TaskAgentNode(BaseNode):
    ...
    async def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # CRÍTICO: Extraer tools_executed del subgrafo para devolverlas al grafo principal
            # En InnerExecutor usamos 'scratchpad' para guardar ToolMessages.
            # Necesitamos convertir esos ToolMessages a la estructura de 'tools_executed'
            # para que el OrchestratorBot pueda reportarlos.

            scratchpad = result.get("scratchpad", [])
            tools_executed_list = []

            # Buscar ToolMessages en el scratchpad
            from langchain_core.messages import ToolMessage
            for msg in scratchpad:
                if isinstance(msg, ToolMessage):
                    tools_executed_list.append({
                        "tool_name": msg.name,
                        "args": "See logs", # Difícil reconstruir sin el AIMessage original
                        "output": msg.content,
            ...
            return {
                ...
                "tools_executed": tools_executed_list, # Esto se sumará al estado global
