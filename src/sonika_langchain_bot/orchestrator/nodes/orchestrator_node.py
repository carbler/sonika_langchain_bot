...
class OrchestratorNode(BaseNode):
    ...
    def _format_chat_history(self, messages: List[Any], limit: Optional[int] = None) -> str:
        """Format conversation history for the prompt."""
        if not messages:
            return "No history."

        # Use full history if limit is None, otherwise slice
        recent = messages if limit is None else messages[-limit:]

        formatted = []
        for msg in recent:
            # Duck typing for custom Message or standard BaseMessage
            is_bot = getattr(msg, "is_bot", False) or isinstance(msg, AIMessage)
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get Chat History (Full History)
        messages = state.get("messages", [])
        chat_history = self._format_chat_history(messages, limit=None)
