...
class PlannerNode(BaseNode):
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Assemble Messages
        # IMPORTANT: We only include the system prompt and the dynamic analysis input.
        # The 'conversation_messages' (history) is usually handled by the state.
        # However, for the ReAct loop to work, we need to persist the intermediate steps.
        # In this architecture, 'messages' in state accumulates the FULL history?
        # If so, we just append to it.

        # Let's assume 'messages' contains the chat history (User, AI, User...)
        # We need to append the CURRENT thought process to it temporarily or permanently?
        # Usually, ReAct steps are ephemeral scratchpads, but for OpenAI function calling they need to be messages.
...
""")
    ...
    def _extract_decision(self, response: AIMessage) -> Dict[str, Any]:
        ...
        if hasattr(response, 'tool_calls') and response.tool_calls:
            ...
            decision["tool_call_id"] = tool_call.get("id") # Guardar ID para el Executor
